...
```bash
node scripts/import-attendance-register.mjs --dry-run
node scripts/import-attendance-register.mjs --commit --actor <admin-profile-uuid>
```

สคริปต์จะหยุดเมื่อจับคู่บุคลากรไม่ได้ พบข้อมูลซ้ำ รหัส/วันที่ผิด หรือยอดจากไฟล์รายวันไม่ตรงกับไฟล์สรุป
