...
- พักหลังเวรดึกอย่างน้อยตามที่ตั้งค่า (default 8 ชม.)
- ห้ามทำงานติดต่อกันเกิน 16 ชม. (เวรบ่ายควบดึก = 16 ชม.พอดี เปิด/ปิดได้ด้วย toggle `allowAfternoonNightDouble`)
...
คะแนนยิ่งต่ำยิ่งควรได้เวรนั้น คำนวณจาก: เวรรวมทั้งเดือน (รวมยอดสะสมข้ามเดือนด้วย — ดูหัวข้อถัดไป) + เวรประเภทเดียวกัน + เวรวันหยุด + วันทำงานติดต่อกัน + โทษการจับคู่คนเดิมซ้ำ (`pairing` weight — ป้องกันคนสองคนถูกจัดเวรเดียวกันซ้ำๆ) ปรับน้ำหนักแต่ละตัวได้ที่หน้าตั้งค่าระบบ
...
ระบบจำจำนวนเวรสะสม**ตลอดชีพ**ของแต่ละคนในทีม (ไม่ใช่แค่เดือนก่อนหน้า) — คนที่เคยได้เวร "ส่วนเกิน" ในเดือนก่อนๆ จะถูกลดความสำคัญลงจนกว่าคนอื่นจะตามทัน ทำให้เวรส่วนเกินหมุนไปเรื่อยๆ ไม่ตกที่คนเดิมซ้ำ คำนวณผ่าน Postgres function `shift_lifetime_totals()` (ไม่ดึงทุกแถวมานับที่แอป — คงที่ตลอดไม่ว่าจะมีประวัติกี่ปี) ส่วนประเภทเวร/Job หมุนเวียนยังอ้างอิงแค่เดือนก่อนหน้าเดือนเดียว (พอสำหรับความลื่นไหลข้ามรอยต่อเดือน)
...
ทั้งสองฟีเจอร์: ตารางต้องไม่ถูกล็อค (เช็คทั้งตอนสร้างคำขอและตอนอนุมัติจริง เผื่อถูกล็อคระหว่างรอ), ต้องอยู่ทีมเดียวกัน, ประวัติทุกคำขอเก็บถาวรไม่มีการลบ (ปุ่มยกเลิก/ปฏิเสธแค่เปลี่ยนสถานะ) หน้าประวัติมีตัวกรองช่วงเดือน + pagination (`components/history-controls.tsx`) โดยรายการที่รอดำเนินการ (รอตอบรับ/รออนุมัติ) จะไม่ถูกซ่อนด้วยตัวกรองหรือ pagination เด็ดขาด
