...
- Monthly OT totals are the primary fairness invariant: when all staff are available, max minus min must be at most 1. Lifetime totals decide who receives the tied extra shift; they must not create an 8-vs-10 split inside one month. Weekend/holiday burden and co-worker pair counts carry forward from the previous month so their rotations do not reset at each boundary.
