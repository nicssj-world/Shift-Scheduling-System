...
8. **Aggregate in Postgres, not in JS, for anything that scans historical rows.** `lib/server/data.ts` `buildCarryIn()` uses the bounded `shift_rolling_fairness()` SQL aggregation for all six-month fairness dimensions (with a temporary bounded fallback for installations before the migration). Follow this pattern for any new "sum/count across history" feature — don't fetch-then-reduce in Node.
...
11. **The scheduler's 16-hour cap includes implicit regular work.** Every non-holiday Monday–Friday has an implicit 08:00–16:00 regular shift for every available employee. It is not OT, is not persisted as an assignment, and does not increase monthly OT totals, but it must participate in continuous-hours, physical work-day, and weekly-day-off checks. The OT-double toggle may allow an A→N double that is exactly 16 hours; it must never permit any chain over 16 hours. Preserve this across month boundaries using `CarryIn.regularWorkDates`.
12. **Publishing and locking are server-validated transitions.** Never trust the client validation result alone. Re-run `validateSchedule()` against the current database assignments inside the state-changing API route, and reject the transition if any hard errors remain. This prevents stale/racing UI state from exposing an invalid roster or sending premature notifications.
13. **Swap/sale concurrency is database-owned.** Never replace the `shift_create_*_request` / `shift_apply_*_request` RPCs with check-then-insert or multiple assignment updates. Active ownership is enforced by the primary key on `shift_assignment_reservations`; apply compares `shift_schedules.assignment_version` and changes assignments + request status atomically. Terminal status triggers release only the reservation. Request rows, items, and `shift_request_events` are permanent audit history and must not be deleted.
...
- When adding a scheduling-fairness change, write a test that would have **failed** before the fix, not just one that passes after — several bugs in this codebase (pairing cliques, rolling carry-in) were only caught by constructing an adversarial fixture and checking the actual distribution, not just "no errors thrown."
- Monthly OT totals are the primary fairness invariant: when all staff are available, max minus min must be at most 1. Rolling six-month totals decide who receives the tied extra shift; they must not create an 8-vs-10 split inside one month. Weekend/holiday burden and co-worker pair counts carry forward over the same six-month window so their rotations do not reset at each boundary.
...
`getActor()` in `lib/server/auth.ts` keeps `Actor.isAdmin` for the actual `profiles.role === 'Admin'` only, while `isScheduler` is `Admin OR user_id present in shift_schedulers`. Use `requireScheduler()` for project-management operations (schedule, teams, shift types, requirements, holidays, approvals, analytics); use `requireAdmin()` for `ตั้งค่าระบบ`, LINE Integration, and delegation/feature-flag administration. `isManager` (role === 'Manager') is intentionally *not* part of `isScheduler`; Manager can view the dashboard but cannot manage the project unless explicitly added to `shift_schedulers`. Attendance register writes use the separate `shift_leave_recorders` list and `requireAttendanceRecorder()`; a designated scheduler is not implicitly a recorder.
